...
import view.MapGridButton;
import view.SimulationListener;
import view.UnitButton;

public class Simulator implements WorldListener, GUIListener {
	...
	public void setSimulationListener(SimulationListener listener) {
		...
		this.simulationListener.setListener(this);
	}
	
	public void createMap() {
		createMapButtons();
		addBuidlingsToMapButtons();
		addCitizensToMapButtons();
		createUnitButtons();
	}
	...
	private void createMapButtons() {
		for(int i = 0;i<10;i++) {
			for(int j = 0;j<10;j++) {
				Address address = this.world[i][j];
				MapGridButton b = new MapGridButton(address);
				this.simulationListener.constructWorld(b);
			}
		}
	}
	
	private void addBuidlingsToMapButtons() {
		for(ResidentialBuilding b : this.buildings) {
			MapGridButton button = this.simulationListener.getMapButtonByAddress(b.getLocation());
			button.getSimulatables().add(b);
		}
	}
	
	private void addCitizensToMapButtons() {
		for(Citizen c : this.citizens) {
			MapGridButton button = this.simulationListener.getMapButtonByAddress(c.getLocation());
			button.getSimulatables().add(c);
		}
	}
	
	private void createUnitButtons() {
		for(Unit unit : this.emergencyUnits) {
			Address address = unit.getLocation();
			UnitButton b = new UnitButton(address, unit);
			this.simulationListener.getUnitButtons().add(b);
		}
	}
	
	public void addExcutedDisastersToMap() {
		this.simulationListener.addExcutedDisastersToMap(this.executedDisasters);
	}
}
